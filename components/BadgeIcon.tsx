...
const BadgeIcon: React.FC<BadgeIconProps> = ({ badge, sizeClass = 'text-2xl', isNew = false }) => {
  return (
    <>
      <div className="relative group flex justify-center">
        <i className={`${badge.iconClass} ${badge.colorClass} ${sizeClass} ${isNew ? 'animate-new-badge-sparkle' : ''} opacity-90 transition-all duration-300 ease-in-out group-hover:scale-125 group-hover:-translate-y-1 group-hover:opacity-100 group-hover:[filter:drop-shadow(0_0_6px_currentColor)]`}></i>
      ...
      {/* Moved style tag to avoid being a direct child of the fragment */}
      <style>
        {`
          ...
          .animate-new-badge-sparkle {
            animation: newBadgeSparkle 1s ease-in-out;
          }
        `}
      </style>
    </>
